...
1. **初回セットアップ**: 起動時に現在の最新ユーザーIDを記録するわ
2. **定期チェック**: 5分ごとに最新10人のユーザーを取得するわ
3. **新規ユーザー検出**: 前回記録したIDと比較して新規ユーザーを抽出するわ
4. **歓迎メッセージ**: 各新規ユーザーにメンション付きで公開投稿するわ
5. **状態更新**: 最新ユーザーIDをSQLiteに保存するわ

## 技術仕様

- **言語**: JavaScript
- **データベース**: SQLite
- **APIクライアント**: misskey-js
- **チェック間隔**: 5分（カスタマイズ可能）
- **レート制限**: メッセージ間に3秒の待機時間
